...
//    
    ...
    {
       $sql="SELECT qm_fse_details.fse_name,qm_fse_details.fse_address,qm_fse_details.fse_email,qm_fse_details.id,qm_fse_location.fse_lat,qm_fse_location.fse_long From qm_fse_details RIGHT JOIN qm_fse_location ON qm_fse_details.id = qm_fse_location.fse_id";
    ...
    public function get_skill_set()
    {
        ...
        $this->db->where('fse_type_status',1);
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $datas = $query->result_array();
            return $datas;
